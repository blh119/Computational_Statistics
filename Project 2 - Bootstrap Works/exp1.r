...
xax<-1:19/20
plot(xax,bsna1[,1], type="o", pch=0, bg=par("bg"), col = "blue",ylim=c(-0.045,0.027),ylab="Bias", cex=.6, main='Bias for EXP1')
...
legend(.2, .02, legend=c("Normal","Plain","Wild","Stu"),
...
#
xax<-1:19/20
plot(xax,bsna1[,2], type="o", pch=0, bg=par("bg"), col = "blue",ylim=c(0.001,0.012),ylab="MSE", cex=.6, main='MSE for EXP1')
...
legend(0.8, 0.012, legend=c("Normal","Plain","Wild","Stu"),
       col=c("blue", "darkgreen","darkorchid","darkorange"),
       lty=1, cex=0.6,text.font=1,box.lty=0)
...
bsna1[,1]  # Normal approx
bsp1   # Plain BS
bss1   # Studentized BS
bsw1   # Wild BS
#bsws   # Stu-Wild BS

expbias <- data.frame(bsp1[,1],bsna1[,1], bss1[,1],
                      bsw1[,1])
names(expbias) <- c('Plain BS Bias', 'Norm BS Bias','Stud BS Bias',
                    'Wild BS Bias')
...
meanexpbias <- c(mean(expbias[,1]),
             mean(expbias[,2]),
             mean(expbias[,3]),
             mean(expbias[,4]))
             #mean(expbias[,5]))

medianexpbias <- c(median(expbias[,1]),
                   median(expbias[,2]),
                   median(expbias[,3]),
                   median(expbias[,4]))


sdexpbias <- c(sd(expbias[,1]),
                sd(expbias[,2]),
                sd(expbias[,3]),
                sd(expbias[,4]))
                #sd(expbias[,5]))

desexpbias <- data.frame(c('Plain BS Bias', 'Norm BS Bias','Stud BS Bias',
                        'Wild BS Bias'), meanexpbias, medianexpbias,
                        sdexpbias)

names(desexpbias) <- c('method', 'mean', 'median', 'standard deviation')


desexpbias

((rep(0:19) + (0.056 < 0.563))/100) - (1:19/20)
